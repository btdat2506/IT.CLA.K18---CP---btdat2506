...
char nfi[]                   =           "NANGSUAT.INP";
char nfo[]                   =           "NANGSUAT.OUT";
...
// Cac bien phu thuoc bao tung bai
int n, dd[202], x[202], a[202][202];
...
int cham() 
{
    ...
    out >> kqo;
    ans >> kqm;
    if (kqm>kqo) return 10; // DS khong lon nhat
    for (i=1; i<=n; i++) dd[i]=0;
    for (i=1; i<=n; i++) out >> x[i];
    // Kiem tra tinh hop le cua x[i]
    for (i=1; i<=n; i++)
    {
        if ((x[i]<1) || (x[i]>n)) return 12; // So hieu viec sai
        if (dd[x[i]]!=0) return 11; // 1 nguoi lam 2 viec
        dd[x[i]]=1;
        if (a[i][x[i]]<kqo) return 13; // Nang suat ho hon DS
    }
    return 0;
}    
...
int viet()
{
    ...
    for (i=Pmin; i<=Pmax; i++)
    {
        ...
        switch (loi[i])
        {
            ...
            case 10: cout << "DS khong lon nhat"; break;
            case 11: cout << "1 nguoi lam 2 viec"; break;
            case 12: cout << "So hieu viec sai"; break;
            case 13: cout << "Nang suat nho hon DS"; break;
        }
        cout << '\n';
    }
    cout << "--------------------------" << '\n';
