// Code sử dụng lập trình cấp cao.
...
struct excls
{
    excls<T, n + 1> operator!() const { return gen(); }
};

template <typename T, uint n>
struct dashes : excls<dashes<T, n>, 0>
{
...
struct L_symbols
{
}; // represents a L|L|L|.. series
template <typename T, uint n>
L_symbols<T, n + 1> operator|(L_symbols<T, n>, Lsymbol) { return gen(); }
...
template <typename T, uint n>
eL_symbols<T, n + 1> operator|(eL_symbols<T, n>, eLsymbol) { return gen(); }
...
template <uint len>
struct line : L_symbols<line<len>, 0>
{
...
template <uint len>
uint const line<len>::length = len;

template <uint x, uint y>
uint const rectangle<x, y>::width = x;
template <uint x, uint y>
uint const rectangle<x, y>::height = y;
template <uint x, uint y>
uint const rectangle<x, y>::area = x *y;

template <uint x, uint y, uint z>
uint const cuboid<x, y, z>::width = x;
template <uint x, uint y, uint z>
uint const cuboid<x, y, z>::height = y;
template <uint x, uint y, uint z>
uint const cuboid<x, y, z>::depth = z;
template <uint x, uint y, uint z>
uint const cuboid<x, y, z>::volume = x *y *z;

template <uint x, uint y, uint z>
rectangle<x, y> front(cuboid<x, y, z>) { return gen(); }
template <uint x, uint y, uint z>
rectangle<z, y> side(cuboid<x, y, z>) { return gen(); }
template <uint x, uint y, uint z>
rectangle<x, z> top(cuboid<x, y, z>) { return gen(); }

// Equality

template <uint ax, uint bx>
bool operator==(line<ax>, line<bx>) { return ax == bx; }

template <uint ax, uint ay, uint bx, uint by>
bool operator==(rectangle<ax, ay>, rectangle<bx, by>)
{
    return ax == bx && ay == by;
}
...
template <uint x>
line<x> operator-(dashes<line_end, x>, line_end) { return gen(); }
// rectangle

template <uint x, uint y>
struct lower_rectangle
{
}; // with lower right corner
...
template <uint x, uint y>
rectangle<x, y> operator|(line<x>, lower_rectangle<x, y>) { return gen(); }

// cuboid
#define create_internet                                  \
    for (ll i = 1; i <= n; i++)                          \
        for (ll j = i + 1; j <= n && j <= i + A[i]; j++) \
            for (ll l = 1; l <= k; l++)                  \
                if (M[i][l - 1] != 0)                    \
                    M[j][l] = max(M[j][l], M[i][l - 1] + C[j]);
template <uint x, uint y, uint z>
struct cuboid_top
{
};
...
operator|(cuboid_top<x, y, z>, excls<line_end, n>)
{
    return gen();
}
...
int main()
{
    ...
    rectangle<2, 3>(o-- ---o | !!!!!o-- ---o);

    cuboid<6, 6, 3>(o-- -----------o | L | L | L | o-- -----------o | !!!!!o | !L | !L | !L | !o-- -----------o);

    cuboid<3, 4, 2>(o-- -----o | L | L | o-- -----o | !!o | !L | !L | !o-- -----o);
}
...
int main()
{
    ...
    vector<vector<ll>> M(n + 1, vector<ll>(k + 1, 0));
    M[1][0] = C[1];
    create_internet
    {
        ll res = 0;
